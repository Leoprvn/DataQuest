...
def test_random_first_last_name_dob(input_value):
    ...
    date_type = type(datetime.datetime.strptime('27/09/2020', '%d/%m/%Y').date())
    result = [create_csv.random_first_last_name_dob(i) for i in type_char]
    print(result)
    ...
    assert type(result[3]) == date_type, 'date is not generated as datetime format'
...
def test_create_csv_file(input_value):
    ...
    assert os.path.exists(file_name) , "File doesn't created, please check the function create_csv_file"
    cnt = 0
    for count in open(file_name):
        cnt += 1
    assert cnt-1 == number_of_lines, 'Number of lines generated is not equal to the numnber of lines passed as parameter'
